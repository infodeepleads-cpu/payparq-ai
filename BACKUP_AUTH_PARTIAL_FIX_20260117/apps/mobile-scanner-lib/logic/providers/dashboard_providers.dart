...
final unifiedDashboardProvider = Provider<AsyncValue<List<Map<String, dynamic>>>>((ref) {
  ...
  List<Map<String, dynamic>> extractData(AsyncValue<List<Map<String, dynamic>>> asyncVal, String type) {
  ...
  if (sessionsAsync.hasError && permitsAsync.hasError) {
    return AsyncValue.error(sessionsAsync.error ?? permitsAsync.error!, StackTrace.current);
  }
  ...
  final filtered = combined.where((item) {
    ...
    final email = (item['email'] ?? item['contact_email'] ?? '').toString().toLowerCase();
    final mobile = (item['mobile'] ?? item['contact_phone'] ?? '').toString().toLowerCase();
    ...
    final isPaid = item['payment_status'] == 'paid' || item['status'] == 'active';
