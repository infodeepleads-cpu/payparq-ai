...
export function ListingCard({ listing, isSelected, onSelect, onBook }: ListingCardProps) {
  return (
    <div
      ...
      className={`p-3 rounded-lg border transition-all cursor-pointer h-[150px] flex flex-col ${
        isSelected ? 'border-[#5F3DFC] bg-[#5F3DFC]/5 shadow-md' : 'border-gray-200 hover:border-gray-300 hover:shadow-sm'
      }`}
    >
      {/* Photo */}
      <div className="relative w-full h-16 rounded-md overflow-hidden mb-2 bg-gray-100 flex-shrink-0">
        <Image
          src={listing.photo}
          alt={listing.name}
          fill
