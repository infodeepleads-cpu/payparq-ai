  ...
  const handleSendEmails = async () => {
    ...
    try {
      ...
      const resolveName = (r: CRMRow): string => {
        ...
        if (r.contact && !isEmail(r.contact)) return r.contact;
        if (r.company && !isEmail(r.company)) return r.company;
        if (r.contact && isEmail(r.contact)) return r.contact.split('@')[0];
        return r.email ? r.email.split('@')[0] : 'Sir/Madam';
      };
