...
export default function VenuePageClient({ venue, slug }: VenuePageClientProps) {
  ...
  return (
    <div className="min-h-screen bg-[#05020A] flex flex-col overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto pt-14 md:pt-16">
                ...
                <h1 className="text-4xl md:text-7xl font-semibold md:font-bold tracking-tight mb-2 md:mb-4 break-words text-white" style={{ lineHeight: '1.2' }}>
                  {locale === 'hr' ? (
                    <>
                      ...
                      <span style={{ color: '#2563eb' }}>{venue.name}</span>
                    </>
                  ) : (
                    <>
        ...
        <AirportReviews airport={slug} locationName={venue.name} />
        <AirportParkingLots airport={slug} lat={venue.lat} lng={venue.lng} airportName={venue.name} />
        <SmarterWayToPark />
        <AirportSEOSection
          location={slug}
        ...
        />
        <NearbyPlaces locationName={venue.name} locationKey={slug} />
        <WhyPayParq airport={venue.name} />
      </main>
