...
export async function POST(req: NextRequest) {
  try {
    ...
    if (role !== 'superadmin' && role !== 'super_admin') {
      return NextResponse.json({ error: 'Only superadmins can create campaigns' }, { status: 403 });
    }

    const body = await req.json();
    const { name, subject, htmlContent, recipientList, scheduleTime } = body;
    ...
    const { data, error } = await supabaseAdmin
      ...
      .insert({
        ...
        recipient_list: recipientList || 'all',
