    ...
    <>
      ...
      <aside className="flex flex-col h-full bg-sidebar border-r border-gray-100 text-black fixed left-0 top-[60px] bottom-0 z-40 w-[60px] pt-2">
        ...
        <div className="px-2 py-2 space-y-2 overflow-visible border-b border-gray-100">
          ...
          {/* Mobile-only Chat Icon - First Place */}
          <div className="md:hidden">
            <Link href={{ pathname: "/" }} className={`group relative flex items-center justify-center p-2 rounded-md transition-colors ${pathname === '/' ? 'bg-gray-100 text-black' : 'text-gray-600 hover:bg-gray-100'}`}>
          ...
          <Link href={{ pathname: "/reminders" }} className={`group relative flex items-center justify-center p-2 rounded-md transition-colors ${pathname.includes('reminders') ? 'bg-gray-100 text-black' : 'text-gray-600 hover:bg-gray-100'}`}>
            <svg className="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
            <span className="absolute left-full ml-3 px-2 py-1 bg-gray-900 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 pointer-events-none">Reminders</span>
          </Link>
